...
| Page | Route | Description |
|------|-------|-------------|
| Login | `/login` | User authentication with username/email + password. **Auto-redirects to dashboard if user has valid token** |
| Self Signup | `/self-signup` | Request access via email (when USER_SIGNUP enabled). **Auto-redirects to dashboard if user has valid token** |
| Forgot Password | `/forgot-password` | Request password reset link. **Auto-redirects to dashboard if user has valid token** |
| Reset Password | `/reset-password?token=...` | Reset password with token. **Auto-redirects to dashboard if user has valid token** |
...
| User Details | `/admin/users/:id` | `USER_MANAGEMENT` | View detailed user information including personal info, role, status, and activity |
...
### 2. Auto-Redirect for Authenticated Users
- Public pages (login, signup, forgot-password, reset-password) automatically check for valid tokens in localStorage
- If a user with a valid token visits a public page, they are automatically redirected to the dashboard
...
- **Click tracking** — Clicking a short URL opens it in a new tab and updates the access count in the background

### 6. Zero Layout Shift on Validation Errors
...
- **Playwright** — Real browser E2E tests covering critical user flows
  - **Login** — Valid/invalid credentials, OTP flow, auto-redirect for authenticated users
  ...
  - **URL Management** — Create, delete, copy URLs with validation, click count tracking
  ...
  - **User Management** — User listing, view details, back navigation, header layout
