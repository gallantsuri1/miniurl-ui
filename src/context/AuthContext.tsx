...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const { clearAuth } = useInitialization();
  ...
  const logout = async () => {
    ...
    // Navigate immediately so user doesn't see stale dashboard
    navigate('/login');

    // Reload global features in background for the login page
    featureService.getGlobalFeatures()
      .then((globalFeatures) => {
